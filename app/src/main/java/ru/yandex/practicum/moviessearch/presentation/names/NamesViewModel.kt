...
class NamesViewModel(private val context: Context, private val namesInteractor: NamesInteractor)
    ...
    private fun searchRequest(newSearchText: String) {
        if (newSearchText.isNotEmpty()) {
            ...
            namesInteractor.searchName(newSearchText, object : NamesInteractor.NameConsumer {
                override fun consume(searchName: List<Person>?, errorMessage: String?) {
                    val persons = mutableListOf<Person>()
                    if (searchName != null) {
                        persons.addAll(searchName)
                    }
